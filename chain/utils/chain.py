...
def get_table_log(sample, skip_op=[], first_n_op=None):
    ...
    for operation in chain:
        ...
        act_func = get_act_func(operation_name)
        table_info = act_func(table_info, operation, skip_op=skip_op)
        if 'row' in operation_name and '_real_select_rows' in table_info:
...
plan_add_column_wiki = '''If the table does not have the needed column to answer the question, we use f_add_column() to add a new column for it. For example,
/*
...
Explanation: The question asks about the number of athletes from japan. We need to known the country of each athlete. There is no column of the country of athletes. We add column "Country of Athlete".'''

plan_add_column_fetaqa = '''If the table does not have the needed column to answer the question, we use f_add_column() to add a new column for it. For example,
/*
col : Rank | Lane | Player | Time
row 1 :  | 5 | Olga Tereshkova (KAZ) | 51.86
row 2 :  | 6 | Manjeet Kaur (IND) | 52.17
...
plan_add_column_tabfact = """If the table does not have the needed column to tell whether the statement is True or False, we use f_add_column() to add a new column for it. For example,
/*
col : rank | lane | player | time
row 1 :  | 5 | olga tereshkova (kaz) | 51.86
row 2 :  | 6 | manjeet kaur (ind) | 52.17
...
plan_select_column_wiki = '''If the table only needs a few columns to answer the question, we use f_select_column() to select these columns for it. For example,
/*
...
plan_select_column_fetaqa = '''If the table only needs a few columns to answer the question, we use f_select_column() to select these columns for it. For example,
/*
col : Code | County | Former Province | Area (km2) | Population | Capital
row 1 : 1 | Mombasa | Coast | 212.5 | 939,370 | Mombasa (City)
row 2 : 2 | Kwale | Coast | 8,270.3 | 649,931 | Kwale
...
plan_select_column_tabfact = """If the table only needs a few columns to tell whether the statement is True or False, we use f_select_column() to select these columns for it. For example,
/*
col : code | county | former province | area (km2) | population | capital
row 1 : 1 | mombasa | coast | 212.5 | 939,370 | mombasa (city)
row 2 : 2 | kwale | coast | 8,270.3 | 649,931 | kwale
...
plan_select_row_wiki = '''If the table only needs a few rows to answer the question, we use f_select_row() to select these rows for it. For example,
/*
...
plan_select_row_fetaqa = '''If the table only needs a few rows to answer the question, we use f_select_row() to select these rows for it. For example,
/*
col : Home team | Home Team Score | Away Team | Away Team Score | Venue | Crowd | Date
row 1 : st kilda | 13.12 (90) | melbourne | 13.11 (89) | moorabbin oval | 18836 | 19 august 1972
row 2 : south melbourne | 9.12 (66) | footscray | 11.13 (79) | lake oval | 9154 | 19 august 1972
...
plan_select_row_tabfact = """If the table only needs a few rows to tell whether the statement is True or False, we use f_select_row() to select these rows for it. For example,
/*
...
plan_group_column_wiki = '''If the question asks about items with the same value and the number of these items, we use f_group_column() to group the items. For example,
/*
...
plan_group_column_fetaqa = '''If the question asks about items with the same value and the number of these items, we use f_group_column() to group the items. For example,
/*
col : District | Name | Party | Residence | First served
row 1 : District 1 | Nelson Albano | Dem | Vineland | 2006
row 2 : District 1 | Robert Andrzejczak | Dem | Middle Twp. | 2013†
...
plan_group_column_tabfact = """If the statement is about items with the same value and the number of these items, we use f_group_column() to group the items. For example,
/*
col : district | name | party | residence | first served
row 1 : district 1 | nelson albano | dem | vineland | 2006
row 2 : district 1 | robert andrzejczak | dem | middle twp. | 2013†
...
plan_sort_column_wiki = '''If the question asks about the order of items in a column, we use f_sort_column() to sort the items. For example,
/*
...
plan_sort_column_fetaqa = '''If the question asks about the order of items in a column, we use f_sort_column() to sort the items. For example,
/*
col : Position | Club | Played | Points
row 1 : 1 | Malaga CF | 42 | 79
row 10 : 10 | CP Merida | 42 | 59
...
plan_sort_column_tabfact = """If the statement is about the order of items in a column, we use f_sort_column() to sort the items. For example,
/*
col : position | club | played | points
row 1 : 1 | malaga cf | 42 | 79
row 10 : 10 | cp merida | 42 | 59
...
plan_full_wiki_simple = '''Here are examples of using the operations to answer the question.
...
plan_full_fetaqa_simple = '''Here are examples of using the operations to answer the question.
...
plan_full_tabfact_simple = """Here are examples of using the operations to tell whether the statement is True or False.
...
def generate_prompt_for_next_step(
    ...
    for operation in possible_next_operations:
        if operation == "<END>":
            continue
        prompt += eval(f"plan_{operation}_{DATASET}") + "\n\n"

    prompt += eval(f"plan_full_{DATASET}_simple") + "\n\n"
...
def dynamic_chain_exec_with_cache_mp(
    all_samples, llm, llm_options=None, strategy="voting", cache_dir="./results/debug", n_proc=10,
    chunk_size=50,
):
    os.makedirs(cache_dir, exist_ok=True)
    result_samples = [None for _ in range(len(all_samples))]
