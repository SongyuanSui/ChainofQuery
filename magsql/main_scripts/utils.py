...
import time
import sqlite3
from magsql.main_scripts.const import subq_pattern
...
def extract_sql(a: str) -> str:
    ...
    if len(sql_list) > 0:
        ...
        for i in range(len(token_list)):
            if "--" in token_list[i]:
                token_list[i] = token_list[i].split("--")[0]
            token_list[i] = token_list[i].strip()
        final_sql = ' '.join(token_list).replace('sql ','')
    else:
        final_sql = None
    return final_sql

def extract_subquery(reply:str) -> list:
    try:
        answer = reply.split('\n')
        subqueries = []
        for i in range(len(answer)):
            if "##" in answer[i]:
                sublist = answer[i].split(":")
                subqueries.append(sublist[1].strip())
    except:
        subqueries = []
    return subqueries

def divide_targets_conditions(reply: str) -> list:
    decomposition = "Targets:" + reply.split("Targets:")[1]
    temp = decomposition.split("Conditions:")
    targets = temp[0].strip()
    conditions = "Conditions:" + temp[1].strip()
    return [targets,conditions]

def extract_new_CoT(reply: str) -> str:
    #extract the new chain of thoughts
    if "**" in reply:
...
def get_matched_content_sequence(matched_contents: dict) -> str:
    """
    Format matched content into a structured string for LLM prompts.

    Input example:
    {
        "table1.columnA": ["value1", "value2"],
        "table1.columnB": ["value3"]
    }
    ...
    for tc_name, contents in matched_contents.items():
        ...
        table_name, column_name = tc_name.split('.', 1)
        column_name = add_quotation_mark(column_name)
        line = f"{table_name}.{column_name} ( " + " , ".join(contents) + " )"
        lines.append(line)
